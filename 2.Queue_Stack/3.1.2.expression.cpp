...
char buffer[max_n + 5];                                      //用字符数组来存表达式
...
void get_value1(Stack * OPND, Stack *OPTR) {
    ...
    while (i < n) {                          //外循环控制整个表达式的读取
        if (buffer[i] > 47 && buffer[i] < 58) {              //如果是数字
            int temp = 0;
            while (buffer[i] > 47 && buffer[i] < 58) {
                temp = temp * 10 + (buffer[i] & 15);
                i++;
            }
            push(OPND, temp);
        } else {                                       //如果是操作符
            if (empty(OPTR) || precede(top(OPTR), buffer[i]) == 0) {     //如果栈空或者这个操作符比栈顶操作符优先级高，就入栈;
                push(OPTR, buffer[i]);
                i++;
            } else if (precede(top(OPTR), buffer[i]) == -1) {             //如果他俩优先级相同说明是(和)，就删除他俩;
                pop(OPTR);
                i++;
